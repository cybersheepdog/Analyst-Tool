...
- **Shared API keys (remote-first, local-fallback).** On the remote backend a team can store its API keys once in the shared database instead of pasting them into every `config.ini`; each analyst loads them automatically and falls back to any local key. Keys can be **encrypted at rest** with a passphrase (Fernet/AES) so the database and its backups only hold ciphertext. Manage them with `analyst_tool_shared_config.py`. See [ANALYST_REMOTE_DB_GUIDE.md](ANALYST_REMOTE_DB_GUIDE.md) and [ADMIN_REMOTE_SERVER_GUIDE.md](ADMIN_REMOTE_SERVER_GUIDE.md).
- **Shared annotations & tags.** Analysts can attach durable notes and tags to an indicator (`>>note 8.8.8.8 confirmed phishing #c2`); they appear automatically at the top of everyone's next lookup. An identical note re-saved within `note_dedup_seconds` (default 60) is skipped, and `>>note-dedupe` clears duplicates already stored, keeping the oldest copy of each. See [NOTE_COMMANDS.md](NOTE_COMMANDS.md).
- **Search & history.** `>>find #c2` searches the whole team's notes/tags (by tag, text, or indicator substring), and `>>history [N] [team]` lists your — or everyone's — recent lookups from the shared check log. Also available via `annotate.py find` / `annotate.py history`.
- **Ticket-ready report export.** `>>report` saves the last lookup(s) — verdict and all service sections, colours stripped, indicators defanged — to a markdown/text file, or straight to the clipboard with `>>report clip`, ready to paste into a SIEM ticket. Configure under `[REPORT]` in `config.ini`.
