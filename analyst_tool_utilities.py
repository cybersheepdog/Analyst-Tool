...
def check_tor(suspect_ip):
    ...
    tor_url = "https://www.dan.me.uk/torlist/?exit"

    # Define number of minutes for age of file
    tor_file_age = 45
    
    # Get current time
    tor_current_time = time.time()
    
    # Calculate time threshold
    tor_threshold_time = tor_current_time - (tor_file_age * 60)  # 60 seconds in a minute

    # check if tor exit node file exists
    if os.path.isfile(tor_exit_nodes_filename):
        #get age of tor exit node file
        tor_file_mod_time = os.path.getmtime(tor_exit_nodes_filename)
        if tor_file_mod_time > tor_threshold_time:
            with open(tor_exit_nodes_filename, 'r') as file:
                #tor_list = file.readlines()
                tor_list = file.read().splitlines()
                if suspect_ip in tor_list:
                    print("\t{:<34} {}".format(color.GREEN + 'TOR Exit Node:' + color.END, "Yes"))
                else:
                    print("\t{:<25} {}".format('TOR Exit Node:', "No"))
        else:
            # get new data and write to file
            try:
                response = requests.request("GET", tor_url)
                if response.status_code == 200:
                    tor_list = response.text.split('\n')
                    with open(tor_exit_nodes_filename, 'w') as file:
                        for ip in tor_list:
                            file.write(f'{ip}\n')
                    if suspect_ip in tor_list:
                        print("\t{:<34} {}".format(color.GREEN + 'TOR Exit Node:' + color.END, "Yes"))
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print("\t{:<25} {}".format('TOR Exit Node:', "Error checking node"))                
        except:
            print("\t{:<25} {}".format('TOR Exit Node:', "Error checking node"))
...
def is_ip_address(clipboardi_contents):
